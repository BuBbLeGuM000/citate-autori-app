...
app.post("/api/quotes/fetch-image", async (req, res) => {
    ...
    try {
        ...
        if (!wikiData.thumbnail?.source) {
            return res.status(404).json({ error: `Nu există imagine disponibilă pentru autorul "${author}" pe Wikipedia.` });
        }
        ...
        if (!imgResponse.ok) {
            return res.status(500).json({ error: "Nu s-a putut descărca imaginea de pe Wikipedia." });
        }
    ...
    try {
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "Ești un cunoscător în literatură și filosofie. Generezi citate scurte, inspiraționale și autentice. Răspunzi DOAR cu citatul, fără ghilimele, fără numele autorului, fără explicații suplimentare. Maxim 2 propoziții."
                },
                {
                    role: "user",
    ...
    try {
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "Ești un asistent concis care descrie personalități istorice. Răspunzi doar în limba română. Răspunsul conține EXACT două propoziții scurte. Menționezi: domeniul, perioada și contribuția principală. Fără introduceri, fără 'Sigur!', fără explicații extra."
                },
                {
                    role: "user",
